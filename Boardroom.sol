...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        uint256 labShare = _balances[msg.sender];
        require(labShare >= amount, "Boardroom: withdraw request greater than staked amount");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = labShare.sub(amount);
...
contract Boardroom is ShareWrapper, ContractGuard {
    ...
    modifier updateReward(address lab) {
        if (lab != address(0)) {
            BoardSeat memory seat = boards[lab];
            seat.rewardEarned = earned(lab);
    ...
    function getLastSnapshotOf(address board) internal view returns (BoardSnapshot memory) {
        return boardHistory[getLastSnapshotIndexOf(board)];
    }
    ...
    function getPartialPrice() external view returns (uint256) {
        return treasury.getPartialPrice();
    }
    ...
    function withdraw(uint256 amount) public override onlyOneBlock boardExists {
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            partial_.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        partial_.safeTransferFrom(msg.sender, address(this), amount);
